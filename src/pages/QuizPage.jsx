...
export default function QuizPage() {
  ...
  // Soft guard: if you have auth, require login; otherwise allow demo take.
  useEffect(() => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      for (const item of quizQuestions) {
        ...
        if (item.type === "text" && !(v && String(v).trim())) {
          throw new Error(`Please answer question ${item.id}.`);
        }
        if (item.type === "single" && !v) {
          throw new Error(`Please answer question ${item.id}.`);
        }
  ...
  const renderQuestion = () => {
    ...
    if (q.type === "multiple") {
      ...
      const toggle = (opt) =>
        setValue(v.includes(opt) ? v.filter((x) => x !== opt) : [...v, opt]);
      return (
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 8 }}>
          {q.options.map((opt) => (
    ...
    <div className="container" style={{ padding: "28px 16px", maxWidth: 900 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div className="subtle" style={{ marginTop: 6 }}>
          Tell us about your goals, personality, and interests so we can match you with
